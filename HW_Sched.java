...
public class HW_Sched {
	...
	public int[] SelectAssignments() {
		...
		ArrayList<Integer> usedDates = new ArrayList<Integer>(lastDeadline);
		
		/*
		 * While there are assignments left in the list 
		 * get that assignment deadline and attempt to complete it on that day (latest possible)
		 * if not possible (look to complete it on an earlier day, if it is not taken already 
		 * if all earlier days are taken then it is not possible so fill with 0
		 */
		while(Assignments.size() != 0) {										
			Assignment a = Assignments.remove(Assignments.size()-1);
			int aDeadline = a.deadline;
			
			for (int count=aDeadline; count >=0; count--) {
				if (count == 0) {
					homeworkPlan[a.number] = 0;
					break;
				}
				if (!usedDates.contains(count)){
					usedDates.add(count);
					homeworkPlan[a.number] = count;
					break;
				}
			}
		}
